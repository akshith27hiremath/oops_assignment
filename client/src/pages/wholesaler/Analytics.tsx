...
const WholesalerAnalytics: React.FC = () => {
  ...
  const loadAnalytics = async () => {
    try {
      setLoading(true);
      const response = await analyticsService.getWholesalerAnalytics(
        dateRange.startDate,
        dateRange.endDate
      );
      ...
      if (response.success) {
        ...
        setTopRetailers(response.data.topRetailers || []);
      }
    } catch (error: any) {
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <p className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">₹{metrics.totalRevenue.toLocaleString()}</p>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            ...
            <p className="text-2xl font-semibold text-gray-900 dark:text-white mt-2">{metrics.activeRetailers}</p>
          </div>

          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Growth Rate</p>
            <p className={`text-2xl font-semibold mt-2 ${metrics.revenueGrowth >= 0 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}`}>
        ...
        {/* Top Retailers */}
        <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6">
          <h2 className="text-lg font-semibold text-gray-900 dark:text-white mb-4">Top Performing Retailers</h2>
          {topRetailers.length > 0 ? (
                ...
                <tbody className="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                  {topRetailers.map((retailer, index) => (
                    <tr key={index}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm text-gray-900 dark:text-white">{retailer.orders}</div>
                      </td>
                      <td className="px-6 py-4 whitespace-nowrap">
                        <div className="text-sm font-semibold text-green-600 dark:text-green-400">₹{retailer.revenue.toLocaleString()}</div>
                      </td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="text-center py-12">
              ...
              <p className="mt-2 text-sm text-gray-500 dark:text-gray-400">No retailer data available</p>
            </div>
          )}
        </div>
      </main>
